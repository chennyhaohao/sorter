...
int main(int argc, char **argv) {
	...
	int r_start=0, r_end=0, argNum=0, attr_num, mode;
	int pipe_name_set = 0, root_pid_set = 0;
	pid_t root_pid  = 1;
	char * usage_msg = 
	"Usage: %s [-f name of file to sort] [-o output file/pipe name] [-s range start] [-e range end] [-a AttrNum] [-r root_pid] [-m mode]\n";
	while ((opt = getopt(argc, argv, "a:f:o:s:e:r:m:")) != -1) { // Use getopt to parse commandline arguments
        switch (opt) {
        case 'f':
...
*/
    ...
    printf("Sort complete. Run time was %lf sec (REAL time) although we used the CPU for %lf sec (CPU time).\n",
